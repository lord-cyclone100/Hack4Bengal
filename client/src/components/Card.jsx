...
export const Card = (props) => {
  const { _id, title, description, hostName, startdate, prizeMoney } = props.val;

  const startDateTemp = new Date(startdate);
  const startDateFinal = startDateTemp.toISOString().split('T')[0];

  // const handleSignIn = async (e) => {
  //   e.preventDefault();

  //   try {
  //     const response = await fetch(`${BACKEND_URL}/participate-in`, {
  //       method: "POST",
  //       headers: { 'Content-Type': 'application/json' },
  //       credentials: "include",
  //       body: JSON.stringify({ gameId: _id, author: hostName._id })
  //     });
    ...
    <div className="w-70 bg-gradient-to-br from-white/5 via-white/10 to-white/20 backdrop-blur-md border border-white/30 rounded-lg shadow-lg">
      ...
      <div className="card-body flex gap-4 font-game">
        ...
        <div class="relative inline-block w-full font-game2 tracking-wide">
          <NavLink to="/join-tournament"><button class="w-full h-10 bg-blue-600 text-white rounded-lg pr-4 text-right">Participate Now</button></NavLink>
